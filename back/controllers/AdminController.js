...
const registro_admin = async function(req, res) {
    ...
    if (admin_arr.length == 0) {
        ...
        if (data.password) {
            ...
            res.status(200).send({ message: 'No existe contraseña', data: undefined });
        }
...
const obtener_nombre_admin = async function(req, res) {
    if (req.user) {
        if (req.user.role == 'admin') {
...
const kpi_ganancias_mensuales_admin = async function(req, res) {
    if (req.user) {
        if (req.user.role == 'admin') {
            ...
            for (var item of reg) {
                let cretedAt_date = new Date(item.createdAt);
                let mes = cretedAt_date.getMonth() + 1;

                if (cretedAt_date.getFullYear() == current_year) {

                    total_ganancia += item.subtotal;

                    if (mes == current_month) {
                        total_mes += item.subtotal;
                        count_ventas += 1;
                    }
